...
> ⏳ **토큰 폐기 — 전체 Refresh 토큰/Access 단축/tokenVersion은 이연**: H-NEW-2의 DB 상태 재검증으로 **탈퇴·휴면 시 즉시 무효화(실질적 폐기)는 이미 달성**. 임의 세션 폐기(예: "모든 기기 로그아웃")용 tokenVersion·짧은 Access+Refresh 회전은 프론트 변경을 동반하는 별도 작업이라 후속(필요 시)으로 분리. 현 MM-프록시 인증 특성상 우선순위 낮음.
...
- **후속(이연)**: Access 토큰 단축 + Refresh, `tokenVersion`/블랙리스트(임의 세션 폐기)는 별도 작업. *(상태 재검증으로 탈퇴/휴면 즉시 폐기는 이미 달성.)*
...
| **Phase B. 보안/계정 하드닝** | 안전성 | H-NEW-2(토큰 상태), JWT_SECRET 검증, M-NEW-6(최초 ADMIN) | ✅ **완료 2026-06-01** (토큰 폐기 전체 Refresh는 이연) |
